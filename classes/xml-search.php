	...
	class SM_XML_Search {
		...
		function init_search() {
			if ( isset( $_GET['sm-xml-search'] ) ) {
				...
				//print_r($input);
				if($input['query_type']=='all' && $input['address']!=''){
					$mainQuery = "INNER JOIN $wpdb->postmeta query_tbl ON query_tbl.post_id = posts.ID AND (query_tbl.meta_key = 'location_address' AND query_tbl.meta_value Like '%".$input['address']."%' || query_tbl.meta_key = 'location_zip' AND query_tbl.meta_value Like '%".$input['address']."%')";
				}elseif($input['query_type']=='all' && $input['state']!='' && $input['state']!='1'){
					$mainQuery = "INNER JOIN $wpdb->postmeta query_tbl ON query_tbl.post_id = posts.ID AND query_tbl.meta_key = 'location_state' AND query_tbl.meta_value Like '%".$input['state']."%'";
				}else{
				...
				$sql = "SELECT 
					...
					INNER JOIN
					$wpdb->postmeta eve_tbl ON eve_tbl.post_id = posts.ID AND eve_tbl.meta_key = 'event_end' AND eve_tbl.meta_value >= CURDATE()
						$mainQuery
						$taxonomy_join
					WHERE
