...
if ( !class_exists( 'Simple_Map' ) ) {
	...
	class Simple_Map {
		...
		function display_map( $atts ) {
			...
			$to_display .= '<section class="map_section"><div id="simplemap" class="map_container" style="' . $hidemap . 'width: ' . $map_width . '; height: ' . $map_height . ';"></div></section>';
			$to_display .= '<div class="content-section"><section class="workshop_by_state"><div id="results" class="result_area max-container" style="' . $hidelist . 'width: ' . $map_width . ';"></div></section></div>';
		...
		function location_search_form( $atts ) {
			...
			// Normal Field inputs
			$ffi['street']		= array( 'label' => apply_filters( 'sm-search-label-street', __( '<div class="title">Find a workshop </div>', 'SimpleMap' ), $post ), 'input' => '<div class="fld_box"><input type="text" id="location_search_address_field" name="location_search_address" placeholder="City or zip code" value="' . esc_attr( $address_value ) . '" /></div></div></div><div id="find" class="searchLeft"><div class="fld_container">' );
			...
			$ffi['submit']		= array( 'label' => '', 'input' => '<div class="col-sm-6 row"><div class="find_btn"><input type="submit" value="' . apply_filters( 'sm-search-label-search', __('GO', 'SimpleMap'), $post ) . '" id="location_search_submit_field" class="submit" /></div></div></div></div>' );
			...
			$hidden_fields[] = '<input type="hidden" id="location_search_state_field" name="location_search_state" value="1" />';
			...
			$location_search  = '<section class="seach_bar text-center"><div class="container"><div id="map_search" class="address_area" >';
			$location_search .= '<a id="map_top"></a>';
			$location_search .= '<form ' . $on_submit . ' name="location_search_form" id="location_search_form" action="' . $action . '" method="' . $method . '"><div class="searchRight"><div class="fld_container">';
			...
			$location_search .= '</div></div></form>';
			$location_search .= '</div></div></section>'; // close map_search div
		...
		function add_distance_field( $radius_value, $units ) {
			...
			// Distance
			$distance_input		= '<div class="fld_box"><div class="row"><div class="col-sm-6"><select id="location_search_distance_field" name="location_search_distance" >';
			...
			$distance_input .= '</select></div>';

			return array( 'label' => apply_filters( 'sm-search-label-distance', __( '<div class="title">Select a distance: </div>', 'SimpleMap' ), $post ), 'input' => $distance_input );
		...
		function enqueue_backend_scripts_styles() {
			...
			if ( is_admin() ) {
				...
				wp_enqueue_style( 'simplemap-map-uistyle', plugins_url() . '/simplemap/css/jquery-ui.css' );
				...
				wp_enqueue_script( 'simplemap-google-api', SIMPLEMAP_MAPS_JS_API . http_build_query( $url_params, '', '&amp;' ) );
				wp_enqueue_script( 'simplemap-master-uistyle', plugins_url() . '/simplemap/js/jquery-ui.js', array( 'jquery' ) );
				wp_enqueue_script( 'simplemap-custom-uistyle', plugins_url() . '/simplemap/js/custom.js', array( 'jquery' ) );
			}
		}
		...
		function google_map_js_script() {
			...
			?>
			var noresults_text			= '<?php echo apply_filters( 'sm-no-results-found-text', __( 'No Upcoming Events.', 'SimpleMap' ) ); ?>';
			...
			function searchLocationsNear(searchData) {
				...
				jQuery.get( searchUrl, {}, function(data) {
					...
					results.innerHTML = "<h3>Workshops By State</h3>";
					...
					/*if(searchData.state!='1'){
						...
						markers.each( function () {
							var ltnData = this;
							console.log(ltnData.state);
							if(ltnData.state!=searchData.state || typeof ltnData.state=='undefined'){
								console.log('enterd',searchData.state);
								markers.splice(k, 1);
							}
							k++;
						});
					...
					if(searchData.address=='' && searchData.state=='1'){
						var newHtml='<div class="col-xs-6 col-sm-3"><div class="workshop"><ul>';
						newHtml += createSidebarEntryState(markers);
...
*/
		...
		,enableEventPropagation: true
			...
			function createSidebarEntryState(markers){
				var html = '';
				var statearray=[];
				markers.each( function () {
					var lcn = this;
					if(statearray.indexOf(lcn.state)=='-1'){
						statearray.push(lcn.state);
					}
				});
				for (var i = 0; i < statearray.length; i++) {
				    if(i>0 && i%8==0){
						html += '</ul></div></div><div class="col-xs-6 col-sm-3"><div class="workshop"><ul>';
					}
					html += '<li><a href="#" class="stateclick">'+statearray[i]+'</a></li>';
				}
				return html;
			}
			...
			function createSidebarEntry(marker, locationData, searchData) {
				...
				// Beginning of result
				var html = '<div id="location_' + locationData.postid + '" class="row">';
				...
				// Name & distance
				html += '<div class="col-sm-9 col-md-9 col-lg-9"><div class="txt">';
				html += '<h3>';
				...
				if (address_format == 'town, province postalcode') {
					...
					html += '<br />' + locationData.zip + ' ' + locationData.city + '-' + locationData.state + '';
				}
				else if (address_format == 'postalcode town, province') {
					html += '<br />' + locationData.zip + ' ' + locationData.city + ', ' + locationData.state + '';
				}
				else if (address_format == 'postalcode town') {
					html += '<br />' + locationData.zip + ' ' + locationData.city + '';
				}
				else if (address_format == 'town postalcode') {
					html += '<br />' + locationData.city + ' ' + locationData.zip + '';
				}
				...
				html += '<br />' + 'Event Start Date:' + ' ' + locationData.evstart + '';
				html += '<br />' + 'Event End Date:' + ' ' + locationData.evend + '';

				html += '</p></div></div>';

				// Links section
				html += '<div class="col-sm-3 col-md-3 col-lg-3"><div class="result_links">';

				// Visit Website link
				
				if (locationData.url != null && locationData.url != 'http://' && locationData.url != '') {
					html += '<div class="result_website btn">';
				...
				if (locationData.distance.toFixed(1) != 'NaN') {
					...
					html += '<div class="btn"><a class="result_directions" href="http://google' + default_domain + '/maps?saddr=' + searchData.homeAddress + '&daddr=' + dir_address + '" target="_blank">' + get_directions_text + '</a></div>';
				}
				html += '</div>';
		...
		function get_default_shortcode_atts() {
			...
			$atts = $tax_atts + array(
				'search_title'				=> __( '', 'SimpleMap' ), 
				'search_form_type'			=> 'table', 
				'search_form_cols'			=> 3, 
				'search_fields'				=> 'labelbr_street||empty||empty||labeltd_distance||empty' . implode('', $tax_search_fields) . '||submit||empty||empty', 
