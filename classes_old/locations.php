...
if ( !class_exists( 'SM_Locations' ) ) {
	class SM_Locations {
		...
		function geo_location( $post ){
			...
			$event_start 		= get_post_meta( $post->ID, 'event_start', true ) ? get_post_meta( $post->ID, 'event_start', true ) : '';
			$event_end 		= get_post_meta( $post->ID, 'event_end', true ) ? get_post_meta( $post->ID, 'event_end', true ) : '';
			...
			?>
				...
				<div class="table">
					<table class="form-table">
						...
						<tr valign="top">
							<td><label for="event_start"><?php _e('Event Start Date', 'SimpleMap'); ?></label></td>
							<td><input type="text" name="event_start" id="event_start" value="<?php echo esc_attr( $event_start ); ?>" /></td>
						</tr>
						<tr valign="top">
							<td><label for="event_end"><?php _e('Event End Date', 'SimpleMap'); ?></label></td>
							<td><input type="text" name="event_end" id="event_end" value="<?php echo esc_attr( $event_end ); ?>" /></td>
						</tr>
		...
		function save_post_meta( $post ) {
			...
			$event_start 		= get_post_meta( $post, 'event_start', true ) ? get_post_meta( $post, 'event_start', true ) : ' ';
			$event_end 		= get_post_meta( $post, 'event_end', true ) ? get_post_meta( $post, 'event_end', true ) : ' ';
			...
			$newevent_start 		= isset( $_POST['event_start'] ) ? $_POST['event_start'] : '';
			$newevent_end 		= isset( $_POST['event_end'] ) ? $_POST['event_end'] : '';
			...
			if ( $event_start != $newevent_start ) update_post_meta( $post, 'event_start', $newevent_start );	
			if ( $event_end != $newevent_end ) update_post_meta( $post, 'event_end', $newevent_end );	
			...
			$updated_event_start = get_post_meta( $post, 'event_start', true );
			$updated_event_end = get_post_meta( $post, 'event_end', true );
